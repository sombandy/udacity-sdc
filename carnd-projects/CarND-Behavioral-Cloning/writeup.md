...
##### Udacity Data
Udacity provided [sample training data](https://d17h27t6h515a5.cloudfront.net/topher/2016/December/584f6edd_data/data.zip) for track1. We will use the Udacity data as the main source of data for training and validation.
...
Total number of data points in this dataset is **738**. The full video of this training data can be seen by clicking on the image below
...
The histogram of the steering angles for this dataset is as follows.
...
The final testing is done on the actual simulator based on whether or not the car is able to drive around on a given track.

###  Architecture and Training Documentation
#### 1. Solution Design Approach
As mentioned at the beginning that we have following goals for the model

1. The model should be able to successfully drive around the car on track1 and also on track2
...
3. **Augment Left/Right Images** Left/right images are augmented after shifting the steering angle
...
We will use the above parameter configuration for all our experiments
...
- Cropping: The upper part of the image shows the sky and trees, not relevant to driving decisions. On the other hand few rows at the bottom of the image mostly show the bonnet of the car. So we crop the input image from 56 to 150 rows
```
img_crop = img[56:150, :, :]
```
...
- Normalization: We apply per-channel normalization by subtracting the mean and dividing by standard deviation of each channel. This will change most pixel values within -1.0 to 1.0, addressing the problem of large input values we saw earlier in case of baseline.
```
